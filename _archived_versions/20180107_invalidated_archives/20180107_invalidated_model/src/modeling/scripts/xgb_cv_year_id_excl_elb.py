...
if not debug_override and __name__ == '__main__' and not "pydevconsole" in sys.argv[0]:
    ...
    parser.add_argument('--cv-n-inner-splits', '-cvis', type=int, default=7)
    parser.add_argument('--cv-n-inner-runs', '-cvir', type=int, default=1)
    parser.add_argument('--xgb-n-threads', '-bnt', type=int, default=16)
    parser.add_argument('--xgb-max-depth-min', '-bmdn', type=int, default=12)
    parser.add_argument('--xgb-max-depth-max', '-bmdx', type=int, default=12)
    parser.add_argument('--xgb-n-rounds', '-br', type=int, default=3000)
...
###
print(f"loading training data to find year ids: {training_data}")
data: DataFrame = pandas.read_pickle(training_data)
...
column_categories = categorical_util.get_categories_lookup(data, remove_unused=True)
categorical_util.set_categories(data, column_categories)
data: pandas.SparseDataFrame = pandas.get_dummies(
    data, prefix_sep='__DUMMY__', dummy_na=True, drop_first=True, sparse=True).to_sparse()
data_feature_names = data.columns.values
data = data.to_coo().tocsr()

print("Saving column categories")
column_cat_path = f"{result_dir}/{run_id}_column_categories.pickle.gz"
...
categorical_util.set_categories(elb_data, column_categories)
elb_data = pandas.get_dummies(
    elb_data, prefix_sep='__DUMMY__', dummy_na=True, drop_first=True, sparse=True
).to_sparse()
elb_data = xgb.DMatrix(elb_data, label=elb_y)
