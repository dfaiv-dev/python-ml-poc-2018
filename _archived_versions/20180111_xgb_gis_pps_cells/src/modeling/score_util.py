...
class ScoreReport:
    def __init__(self, y, predictions):
        ...
        self.abs_99 = self.abs_mean + (self.abs_std * 3)
        self.percent_mean = err_percent_mean
        self.percent_std = err_percent_std
        self.percent_max = err_percent_max

    def __str__(self):
        return """SCORE REPORT >> abs99: {:.2f} (r2: {:.6f}) <<
abs_mean: {:.2f}, abs_std: {:.2f}, abs_99: {:.2f}, abs_max: {:.2f}
r2: {:.6f}, sq_mean: {:.2f}, abs_median: {:.2f}""".format(
            self.abs_99, self.r2,
            self.abs_mean, self.abs_std, self.abs_99, self.abs_max,
