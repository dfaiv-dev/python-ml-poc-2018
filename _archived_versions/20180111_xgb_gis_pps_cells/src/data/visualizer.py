...
def plot_all_pcsml(df: pd.DataFrame, out_dir: str, rug=False, max_categories=20):
    ...
    for col in [c for c in df.columns if c not in exclude_cols]:
        print("plotting dists: %s" % col)

        if df[col].dropna().count() == 0:
            print(f"WARNING: {col} is all NaN, nothing to plot")
            continue

        plot_distribution(df, col, out_dir, rug, max_categories)


def plot_distribution(df: pd.DataFrame, col: str, out_dir: str, rug=False, max_categories=20):
    kde = True
    if len(df[col].dropna().unique()) == 1:
        print(f">> {col} has only one unique value, kde=False")
        kde = False
...
def _save_plot(out_dir: str, name: str, close=True, dpi=150):
