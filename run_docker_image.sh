...
NO_SHUTDOWN=${no_shutdown-"-"}
 ...
 # ahoffmanSubscription                            49c7d034-2a69-4cac-b9c7-fae4e8092034
# Visual Studio Professional - David Faivre        2b1f92db-0edf-4317-94ab-53eca82df9dc
# MSDN - VS Prof - DMF - 2017-03                   d1d829db-9a6e-4dec-a028-195c49c8871a
# greg osb:                                        7090af32-6d37-4671-abdf-9a1aaf2d3b66
VM_SUB_ID='7090af32-6d37-4671-abdf-9a1aaf2d3b66'
...
function _set_sub() {
    ...
    _subId=$VM_SUB_ID
    echo "setting default subscription: ${VM_SUB_ID}"
    az account set --subscription $_subId
    if [[ ! $? -eq 0 ]]; then
        echo "error: trying logout and login to reset credentials"
        az logout
        az login
        az account set --subscription $_subId
    fi
...
# not the -v instead of mnt for the temp so that the dir gets created
# see https://docs.docker.com/engine/admin/volumes/bind-mounts/#start-a-container-with-a-bind-mount
_docker_run_opts=\
...
ssh -oStrictHostKeyChecking=no -t [devusername]@$_vm_ip "bash ~/remote-exec/run-dkr-image.sh \
    ...
    ~/remote-exec "\""$VM_RESOURCE_GRP"\"" "\""$VM_NAME"\"" "\""$VM_SUB_ID"\"" \
...
#ssh -t [devusername]@$VM_DNS_NAME \
#"bash ~/remote-exec/run-tmux.sh debug-session \". ~/remote-exec/run-and-shutdown.sh -c 'echo hello!; sleep 5; echo 